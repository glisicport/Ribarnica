...
@if (session('status'))
    <p class="mb-4 text-green-700 bg-green-100 px-4 py-2 rounded">{{ session('status') }}</p>
@endif
            ...
            <tbody>
                @foreach ($questions as $q)
                    <tr class="border-t border-gray-200 hover:bg-gray-50">
                        <td class="px-4 py-2 text-gray-700">{{ $q->created_at->format('d.m.Y. H:i') }}</td>
                        <td class="px-4 py-2 text-gray-700">{{ $q->username }}</td>
                        <td class="px-4 py-2 text-gray-700">{{ $q->message }}</td>
                        ...
                        <td class="px-4 py-2 flex flex-col gap-2">
                            <form method="POST" action="{{ route('kontrolni-panel.update', ['resource' => 'questions', 'id' => $q->id]) }}">
                                @csrf
                                @method('PUT')
                                <textarea name="comment" rows="2" class="w-full border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-400">{{ $q->comment }}</textarea>
