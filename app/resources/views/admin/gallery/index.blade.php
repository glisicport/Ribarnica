            ...
            @foreach($files as $file)
                ...
                <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden hover:shadow-2xl transition duration-300">
                    ...
                    <div class="p-4 flex flex-col justify-between min-h-[100px]">
                        <span class="text-gray-900 font-semibold truncate">{{ pathinfo($fileName, PATHINFO_FILENAME) }}</span>
                        <div class="flex justify-end gap-2 mt-3">

                            {{-- DUGME IZMENI SLIKU --}}
                            <button onclick="openEditModal('{{ $fileName }}')"
                            ...
                            {{-- DUGME OBRIŠI --}}
                            <form action="{{ route('admin.gallery.delete', $fileName) }}" method="POST">
                                @csrf
                                @method('DELETE')
    ...
    <form method="POST" id="renameImageForm">
        ...
        <select name="stari_naziv" id="renameSelect" class="w-full border rounded-lg p-2 mb-4" required>
            ...
            @foreach($folderFiles as $file)
                <option value="{{ $file->getFilename() }}">{{ pathinfo($file->getFilename(), PATHINFO_FILENAME) }}</option>
            @endforeach
        </select>
